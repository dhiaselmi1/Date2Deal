...
GEMINI_API_KEY = "AIzaSyAEvC463-b_vKQsnA3IGX_2pKHPDRqzkjs"
...
def find_company_website(company_name, region=None):
    """Trouve le site officiel - Requêtes multiples pour plus d'efficacité"""
    
    # Essayer plusieurs stratégies de recherche
    queries = [
        f'{company_name}',  # Recherche simple - souvent la plus efficace
        f'{company_name} site:*.com OR site:*.fr OR site:*.org',  # Forcer les domaines officiels
        f'"{company_name}" -linkedin -facebook -wikipedia -zoominfo',  # Exclure explicitement
    ]
    
    if region:
        queries.insert(0, f'{company_name} {region}')  # Ajouter la région en premier
    
    print(f"🔍 Recherche du site officiel de {company_name}...")
    
    # Sites à éviter
    excluded_sites = ['linkedin', 'facebook', 'twitter', 'wikipedia', 'indeed', 
                     'zoominfo', 'glassdoor', 'instagram', 'youtube']
    
    for query_num, query in enumerate(queries, 1):
        print(f"\n  📝 Essai {query_num}: {query}")
        ...
        try:
            ...
            for i, item in enumerate(data.get("organic", []), 1):
                ...
                print(f"    [{i}] {title[:50]}... -> {link}")
                
                # Vérifier si ce n'est pas un site exclu
                if not any(excluded in link.lower() for excluded in excluded_sites):
                    # Vérifier que c'est bien un site web (pas un PDF, etc.)
                    if link.startswith('http') and not link.endswith('.pdf'):
                        print(f"✅ Site officiel trouvé (requête {query_num}, position {i}): {link}")
                        return link
                        
        except Exception as e:
            print(f"    ❌ Erreur pour cette requête: {e}")
    
    print("❌ Aucun site officiel trouvé dans toutes les recherches")
    return None
...
def extract_leaders_from_content(content, company_name, website_url,region):
...
- Région cible : "{region}"
- IGNORE complètement toute information concernant d'autres régions/pays
- Si l'URL contient /americas/, /europe/, /asia/ et que la région cible est "{region}", IGNORE ce contenu
- EXCEPTION : Si l'URL contient "global" ET ne contient aucune autre région spécifique, ANALYSE le contenu
- Les URLs "global" sans indication de région sont acceptées (ex: /global/fr/ OK, /global/americas/ NON)
- Concentre-toi UNIQUEMENT sur les dirigeants de la région "{region}" ou les postes globaux pertinents
...
- La personne doit être basée en "{region}" ou dans un poste global pertinent
- Ignorer les témoignages clients ou partenaires externes
- IGNORER tous les dirigeants d'autres régions/pays

FORMAT DE SORTIE OBLIGATOIRE :
Nom Complet | Poste Exact | Email (si disponible) | Département

Si aucun dirigeant n'est trouvé pour la région "{region}", écris : "AUCUN DIRIGEANT IDENTIFIÉ POUR {region.upper()}"

DIRIGEANTS IDENTIFIÉS POUR {region.upper()} :
"""
...
def search_linkedin_profiles(person_name, company_name, region=None):
    ...
    return {}
def search_linkedin_profiles1(person_name, company_name, region=None, role=None):
    """Recherche et vérifie les profils LinkedIn"""

    query = f'"{person_name}" "{role}" "{company_name}"  site:linkedin.com'
    if region:
        query += f' "{region}"'
...
def comprehensive_leader_search(company_name, region, min_people=15):
    ...
    for i, page_url in enumerate(team_pages):
        ...
        leaders_text = extract_leaders_from_content(page_content, company_name, page_url,region)
    ...
    for query in search_queries:
        ...
        try:
            ...
            for line in extracted.splitlines():
                ...
                if not validate_person_data({'name': name, 'role': role}, company_name):
                    continue

                # **Nouvelle étape** : recherche du profil LinkedIn réel
                linkedin = search_linkedin_profiles(name, company_name, region)
                url_linkedin = linkedin.get("linkedin_url", "")
                if not bool(url_linkedin):
                  url_linkedin=search_linkedin_profiles1(name, company_name, region,role )
                leaders.append({
                    'name': name,
                    'role': role,
                    'email': '',
                    ...
                    'region': region,
                    'source': 'LinkedIn',
                    'source_url': url_linkedin,
                    'linkedin_url': url_linkedin,
                    'linkedin_verified': bool(url_linkedin),
                    'category': categorize_role(role)
                })
                processed_names.add(name.lower())
                print(f"✅ LinkedIn validé: {name} → {url_linkedin or '(non trouvé)'}")

                time.sleep(1)  # pour respecter le rate limit
...
if __name__ == "__main__":
    ...
    print("🔹 Garantit minimum 15 dirigeants validés")
    print("=" * 70)
